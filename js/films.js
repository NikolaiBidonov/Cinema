...
for (let i = 0; i < filmsHire.length; i++) {
    ...
    orderFilmButton.onclick = function (event) {
        ...
        if (orderClientName.value && orderClientPhone.value) {
            ...
            console.log('Количество билетов', orderCountTicket.innerText);
...
p.then((data) => {
    ...
    <div class="films-grid__inner">
        ...
        <div class="films-grid__description">
            ...
            <p class="films-grid__text2">Дата релиза: ${itemRelease}</p>
            <div>
                <a href="${linkFb}" title="facebook" target="_blank">
                ...
                <a href="${linkBehance}" title="behance" target="_blank">
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                        viewbox="0 0 34 34">
                        <defs>
                            <filter filterUnits="userSpaceOnUse" id="Filter_0" x="0px" y="0px"
                ...
                <a href="${linkDribble}" title="dribbble" target="_blank">
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                        viewbox="0 0 34 34">
                        <defs>
                            <filter filterUnits="userSpaceOnUse" id="Filter_0" x="0px" y="0px"
        ...
        itemElement.innerHTML = itemSlider;
        sliderSection.appendChild(itemElement)
    }
})

let arr = [],
    price,
    regularSeatsPrice = 100,
    goodSeatsPrice = 200,
    isAvailable;

// Создание зрительного зала. Занятые места генерируются рандомно, места в центре
// зала по 200 руб, по бокам - 100 руб
function showCinemaHall(count) {

    for (let i = 1; i <= count; i++) {
        isAvailable = Math.round(Math.random() * (10 - 0) + 0)
        if (isAvailable > 5) {
            isAvailable = true
        } else {
            isAvailable = false
        ...
        if (i > 3 && i < 8) {
            price = goodSeatsPrice
        } else {
            price = regularSeatsPrice
        }
        arr.push({
            'number': i,
            'price': price,
...
let numberInput = document.querySelector('#orderFilmButton')
let orderFilmSeat = document.querySelector('#orderFilmSeat'),
    orderFilmTotalPrice = document.getElementById('orderFilmTotalPrice'),
    check = false

//Отображение массива в форме и добавление событий местам

for (let i = 0; i < arr.length; i++) {
    let divElement = document.createElement('div')
    divElement.innerText = arr[i].number
    divElement.price = arr[i].price
    divElement.number = arr[i].number
    divElement.classList.add('seats-element')
    if (arr[i].available === true) {
        divElement.classList.add('seats-available')
    } else {
        divElement.classList.add('seats-not-available')
    }
    divElement.addEventListener('mouseover', placeHover)
    divElement.addEventListener('mouseout', placeHoverOut)
    divElement.addEventListener('click', order)
    divElement.addEventListener('click', placeToggle)
    divElement.addEventListener('contextmenu', placeContext)
    seats.append(divElement)
}

//Изменение цвета при наведении
function placeHover(e) {
    e.target.classList.add('seats-hover')
}

function placeHoverOut(e) {
    e.target.classList.remove('seats-hover')
}

//Изменение цвета при выборе места
function placeToggle(e) {
    if (e.target.classList.contains('seats-available') === true) {
        e.target.classList.toggle('chosen-place')
    }
}

//Вывод стомости билета при нажатии правой кнопкой мыши по месту
function placeContext(e) {
    if (this.classList.contains('seats-available') === true) {
    e.preventDefault()
    alert(`Стоимость билета ${this.price} ₽`)
    }
}

// Отображение выбранного места и стоимости билета в форму и вывод
// предупреждения, если место занято
function order(e) {
    
    if (e.target.classList.contains('seats-not-available') === true) {
        alert('Это место занято. Пожалуйста, выберите другое место')
    } else if (check === true) {
        check = false
        orderFilmTotalPrice.innerText = '';
        orderFilmSeat.innerText = '';
    } else if (e.target.classList.contains('seats-available') === true) {
        orderFilmTotalPrice.innerText = e.target.price
        orderFilmSeat.innerText = e.target.number
        check = true
    }
}

// let parentDiv = document.querySelector('.film-order__button')
// parentDiv.insertBefore(seats, numberInput)
$(function() {
    $(seats).insertAfter('#numberInput')
})
